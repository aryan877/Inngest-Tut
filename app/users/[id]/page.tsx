...
export default function UserProfilePage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      {/* User Header */}
      <div className="bg-white dark:bg-zinc-800 p-8 mb-8 rounded shadow-sm">
        <div className="flex items-start gap-6">
          <Avatar className="h-24 w-24">
            <AvatarImage src={user.image || undefined} alt={user.name} />
            <AvatarFallback className="text-2xl bg-zinc-100 dark:bg-zinc-700 text-zinc-800 dark:text-white">
              {user.name.charAt(0).toUpperCase()}
            </AvatarFallback>
          </Avatar>

          <div className="flex-1">
            <h1 className="font-outfit text-3xl font-bold mb-2 text-zinc-800 dark:text-white">
              {user.name}
            </h1>
            {profile?.bio && (
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-sm">
                ...
                {profile.website && (
                  <a
                    ...
                    className="text-zinc-800 dark:text-zinc-100 underline hover:no-underline"
                  >
                    🔗 {profile.website}
                  </a>
                )}
          ...
          <div className="space-y-4">
            {questions && questions.length > 0 ? (
              questions.map((question: any) => (
                <div
                  key={question.id}
                  className="bg-white dark:bg-zinc-800 p-4 border border-zinc-200 dark:border-zinc-700 rounded hover:shadow-sm transition-shadow"
                >
                  <Link href={`/questions/${question.id}`}>
                    <h3 className="font-semibold mb-2 text-zinc-800 dark:text-white hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
                      {question.title}
                    </h3>
                  </Link>
              ...
              <p className="text-zinc-700 dark:text-zinc-400">No questions yet</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {answers && answers.length > 0 ? (
              answers.map((answer: any) => (
                ...
                >
                  <Link href={`/questions/${answer.question.id}`}>
                    <div className="text-sm text-zinc-700 dark:text-zinc-400 mb-1">
                      Answered: {answer.question.title}
                    </div>
                  </Link>
                  <div className="text-sm text-zinc-700 dark:text-zinc-400 mb-2 line-clamp-2">
              ...
              <p className="text-zinc-700 dark:text-zinc-400">No answers yet</p>
            )}
          </div>
        </div>
      </div>
